...
export default function App() {
  ...
  const dynamicCss = useMemo(() => {
    ...
    const isDark = bgPage === "#0f172a" || bgPage === "#18181b" || bgPage === "#090d16" || bgPage === "#0a0a0a" || bgPage === "#050c05" || bgPage === "#121319" || bgPage === "#171923" || bgPage === "#0a0b0e" || bgPage === "#1e2235";
...
function AdminView({ 
  ...
  const exportDatabase = async () => {
    ...
    try {
      ...
      // Row 1 sheet data: Opdrachtgevers list
      const clientsSheetData = clients.map(client => ({
        "Naam opdrachtgever": client.name || "",
        "Prijs Uitlezen (€)": client.priceUitlezen ?? 0,
        "Eenheid Uitlezen (Ae of €)": client.unitUitlezen ?? "€",
        "Prijs Uitlijnen (€)": client.priceUitlijnen ?? 0,
        "Prijs Koelvloeistof (€)": client.priceKoelvloeistof ?? 0,
        "Prijs Antiroest (€)": client.priceAntiroest ?? 0,
        "Prijs Portierfolie (€)": client.pricePortierfolie ?? 0,
        "Prijs Dempingsmatten (€)": client.priceDempingsmatten ?? 0,
        "Link Prijslijst": client.priceListLink ?? ""
      }));

      const wsClients = XLSX.utils.json_to_sheet(clientsSheetData);
      XLSX.utils.book_append_sheet(wb, wsClients, "Opdrachtgevers");

      // For each client, fetch their specific custom part-prices and add as subsequent sheets
      for (const client of clients) {
        const pDocs = await getDocs(collection(db, "clients", client.id, "prices"));
        const pricesData = pDocs.docs.map(d => ({
          "Onderdeelnummer": d.data().partNumber || "",
          "Omschrijving": d.data().description || "",
          "Afgesproken Prijs (€)": d.data().price || 0
        }));

        // Limit sheet name to 30 characters (Excel sheets have a 31 char limit) & filter invalid characters
        const safeSheetName = client.name.replace(/[*?:\\/\[\]]/g, "").slice(0, 30) || `Prijslijst_${client.id.slice(0, 4)}`;
        const wsPrices = XLSX.utils.json_to_sheet(pricesData);
        XLSX.utils.book_append_sheet(wb, wsPrices, safeSheetName);
      }

      // Write file
      XLSX.writeFile(wb, `partverify_opdrachtgevers_backup_${new Date().toISOString().split('T')[0]}.xlsx`);

      setLocalToast("Excel database succesvol geëxporteerd!");
      setTimeout(() => setLocalToast(null), 3000);
    } catch (err) {
      console.error(err);
      alert("Fout bij exporteren database naar Excel!");
    } finally {
      setIsExporting(false);
    }
  };

  const importDatabase = async (fileBuffer: ArrayBuffer, mode: "merge" | "overwrite") => {
    setIsImporting(true);
    ...
    try {
      ...
      if (!firstSheetName) {
        throw new Error("Het geselecteerde Excel bestand bevat geen werkbladen.");
      }

      // First sheet acts as "Opdrachtgevers" settings
      const clientsSheet = workbook.Sheets[firstSheetName];
      const clientsRows = XLSX.utils.sheet_to_json<any>(clientsSheet);

      if (clientsRows.length === 0) {
        throw new Error("De eerste sheet (Opdrachtgevers) in het Excel bestand is leeg of ontbreekt.");
      }

      // Store other sheets' data for custom part pricing collections
      const otherSheetsData: Record<string, any[]> = {};
      for (let i = 1; i < workbook.SheetNames.length; i++) {
        const name = workbook.SheetNames[i];
        otherSheetsData[name.toLowerCase().trim()] = XLSX.utils.sheet_to_json<any>(workbook.Sheets[name]);
      }

      setImportStatus("Gegevens valideren...");
      for (const row of clientsRows) {
        const nameProp = row["Naam opdrachtgever"] || row["Naam"] || row["Name"] || row["name"];
        if (!nameProp) {
          throw new Error("Geen geldige naam kolom gevonden in een van de rijen op de 'Opdrachtgevers' sheet.");
        }
      }

      // If overwriting, clear the existing database first
      if (mode === "overwrite") {
      ...
      setImportStatus("Opdrachtgevers uit Excel importeren...");
      let importedCount = 0;
      let pricesCount = 0;

      for (const row of clientsRows) {
        const nameProp = String(row["Naam opdrachtgever"] || row["Naam"] || row["Name"] || row["name"]).trim();
        let clientId = "";
        let existingClient = mode === "merge" ? clients.find(c => c.name.toLowerCase() === nameProp.toLowerCase()) : null;

        // Parse individual columns from Dutch Excel headers
        const priceUitlezen = parseFloat(String(row["Prijs Uitlezen (€)"] || row["Prijs Uitlezen (€/Ae)"] || row["Prijs Uitlezen"] || 0)) || 0;
        const unitUitlezen = String(row["Eenheid Uitlezen (Ae of €)"] || row["Eenheid Uitlezen"] || "€").trim() === "Ae" ? "Ae" : "€";
        const priceUitlijnen = parseFloat(String(row["Prijs Uitlijnen (€)"] || row["Prijs Uitlijnen"] || 0)) || 0;
        const priceKoelvloeistof = parseFloat(String(row["Prijs Koelvloeistof (€)"] || row["Prijs Koelvloeistof"] || 0)) || 0;
        const priceAntiroest = parseFloat(String(row["Prijs Antiroest (€)"] || row["Prijs Antiroest"] || 0)) || 0;
        const pricePortierfolie = parseFloat(String(row["Prijs Portierfolie (€)"] || row["Prijs Portierfolie"] || 0)) || 0;
        const priceDempingsmatten = parseFloat(String(row["Prijs Dempingsmatten (€)"] || row["Prijs Dempingsmatten"] || 0)) || 0;
        const priceListLink = String(row["Link Prijslijst"] || row["Link"] || row["priceListLink"] || "").trim();
        ...
        // Match sheet name with client name to extract their unique price lists
        const matchedSheetName = Object.keys(otherSheetsData).find(
          s => s === nameProp.toLowerCase().trim() || s.replace(/[^a-z0-9]/g, "") === nameProp.toLowerCase().replace(/[^a-z0-9]/g, "").trim()
        );

        if (matchedSheetName) {
          const sheetRows = otherSheetsData[matchedSheetName];
          setImportStatus(`Prijslijst inladen voor ${nameProp}...`);

          // Clear previous prices on merge/overwrite to prevent stale price matches
          if (existingClient) {
          ...
          for (const pr of sheetRows) {
            const partNumber = String(pr["Onderdeelnummer"] || pr["PartNumber"] || pr["partNumber"] || pr["Onderdeel"] || "").trim();
            const description = String(pr["Omschrijving"] || pr["Description"] || pr["description"] || "").trim();
            const price = parseFloat(String(pr["Afgesproken Prijs (€)"] || pr["Prijs"] || pr["Price"] || pr["price"] || 0)) || 0;
      ...
      setLocalToast(`Succesvol ${importedCount} nieuwe/bijgewerkte opdrachtgevers en ${pricesCount} prijsafspraken geïmporteerd!`);
      setTimeout(() => setLocalToast(null), 4500);
      loadData();
    } catch (err: any) {
      console.error(err);
      alert(`Excel import mislukt: ${err?.message || "Ongeldig formaat of syntaxis foute kolommen."}`);
      setImportStatus(null);
    } finally {
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-white rounded-3xl border border-slate-200 p-6 space-y-4">
              <div className="flex items-center gap-2 text-slate-800">
                ...
                <h3 className="text-sm font-black uppercase tracking-widest text-slate-700">Database Beheer (Excel)</h3>
              </div>
              <p className="text-xs text-slate-500 font-medium leading-relaxed">
                Exporteer uw volledige database (opdrachtgevers én alle gekoppelde prijslijsten) als een overzichtelijk Excel-bestand, of laad een vooraf ingevuld Excel-bestand in.
              </p>
              ...
              >
                <FileDown size={14} />
                {isExporting ? "Aan het exporteren..." : "Exporteer Database (Excel)"}
              </button>
              ...
              <div className="space-y-3">
                ...
                <label className="flex items-center justify-center border border-dashed border-slate-300 rounded-xl p-4 hover:border-slate-400 cursor-pointer text-xs font-bold text-slate-500 bg-slate-50/50 hover:bg-slate-50 transition-all min-h-[64px] text-center">
                  <div className="flex flex-col items-center gap-1.5">
                    ...
                    <span>Selecteer database (.xlsx, .xls)</span>
                  </div>
                  <input
                    type="file"
                    ...
                    onChange={(e) => {
                      ...
                      const confirmMsg = importType === "overwrite"
                        ...
                        : "Weet u zeker dat u de gekozen Excel database wilt inladen en samenvoegen met uw bestaande opdrachtgevers?";
              ...
              <details className="text-[10px] text-slate-400 cursor-pointer hover:text-slate-600">
                <summary className="font-semibold select-none">Bekijk Excel lay-out structuur</summary>
                <div className="mt-2 p-3 bg-slate-50 border border-slate-100 rounded-lg text-[10px] space-y-2 text-slate-600 leading-relaxed font-semibold">
                  <p className="text-amber-700 font-extrabold uppercase text-[9px] tracking-wider mb-0.5">Blad 1: "Opdrachtgevers"</p>
                  <p className="pl-1.5 mb-2">
                    Kolommen: <br />
                    <code className="font-mono bg-white px-1 py-0.5 rounded border border-slate-200">Naam opdrachtgever</code> * <br />
                    <code className="font-mono bg-white px-1 py-0.5 rounded border border-slate-200">Prijs Uitlezen (€)</code> <br />
                    <code className="font-mono bg-white px-1 py-0.5 rounded border border-slate-200">Eenheid Uitlezen (Ae of €)</code> <br />
                    <code className="font-mono bg-white px-1 py-0.5 rounded border border-slate-200">Prijs Uitlijnen (€)</code> <br />
                    <code className="font-mono bg-white px-1 py-0.5 rounded border border-slate-200">Prijs Koelvloeistof (€)</code> <br />
                    <code className="font-mono bg-white px-1 py-0.5 rounded border border-slate-200">Link Prijslijst</code>
                  </p>
                  
                  <div className="border-t border-slate-200 my-1.5"></div>
                  
                  <p className="text-amber-700 font-extrabold uppercase text-[9px] tracking-wider mb-0.5">Blad 2, 3, enz. (vrij te benoemen naar opdrachtgever):</p>
                  <p className="pl-1.5 leading-relaxed">
                    Als u tabbladen toevoegt die precies overeenkomen met de naam van de opdrachtgever (bijvoorbeeld <code className="font-mono bg-white px-1 py-0.5 rounded border border-slate-200">Allianz</code>), worden de specifieke onderdeelafspraken automatisch ingeladen!
                  </p>
                  <p className="pl-1.5">
                    Kolommen: <br />
                    <code className="font-mono bg-white px-1 py-0.5 rounded border border-slate-200">Onderdeelnummer</code> * <br />
                    <code className="font-mono bg-white px-1 py-0.5 rounded border border-slate-200">Omschrijving</code> <br />
                    <code className="font-mono bg-white px-1 py-0.5 rounded border border-slate-200">Afgesproken Prijs (€)</code> *
                  </p>
                  <p className="text-[9px] text-slate-400 italic font-medium mt-1">(* verplicht veld)</p>
                </div>
              </details>
            </div>
          </div>
